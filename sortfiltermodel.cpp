...
{
    m_sortRoles.insert( Roles::RssiRole );
}
...
bool SortFilterModel::lessThan(const QModelIndex &left, const QModelIndex &right) const
{
    ...
    if (m_sortRoles.contains(Roles::RssiRole)) {
        const auto leftVal = left.data(Roles::RssiRole).toInt();
        const auto rightVal = right.data(Roles::RssiRole).toInt();
        if (leftVal != rightVal)
            return leftVal > rightVal;
    }
